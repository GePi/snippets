*----------------------------------------------------------------------*
*       CLASS lcl_alv_viewer DEFINITION
*----------------------------------------------------------------------*
*
*----------------------------------------------------------------------*
CLASS lcl_alv_viewer DEFINITION ABSTRACT.
  PUBLIC SECTION.
    ...
    METHODS:
      constructor IMPORTING io_controller TYPE REF TO linf_report,
    ...
    DATA:
      mo_controller TYPE REF TO linf_report.
...
*----------------------------------------------------------------------*
* CLASS lcl_qrmin_viewer DEFINITION
*----------------------------------------------------------------------*
* Конкретный класс
*----------------------------------------------------------------------*
CLASS lcl_qrgen_viewer DEFINITION INHERITING FROM lcl_alv_viewer FINAL.
  PUBLIC SECTION.
    METHODS:
      constructor
        IMPORTING io_controller TYPE REF TO linf_report,
    display.
...
ENDCLASS.                    "lcl_qrmin_viewer DEFINITION
...
CLASS lcl_alv_viewer IMPLEMENTATION.
  ...
  METHOD control_init.
    ...
    md_data_table = mo_controller->get_ref_to_data( ).
  ...
  METHOD fill_variant.
    ...
    CASE abap_true.
      WHEN rb_qrmin.
        rs_var-report = |{ sy-repid } _1 |.
      WHEN rb_qrgen.
        rs_var-report = |{ sy-repid } _2 |.
      WHEN rb_list.
        CASE abap_true.
          WHEN rb_matr.
            rs_var-report = |{ sy-repid } _3M |.
          WHEN rb_equip.
            rs_var-report = |{ sy-repid } _3E |.
        ENDCASE.
      WHEN rb_usage.
        CASE abap_true.
          WHEN rb_matr.
            rs_var-report = |{ sy-repid } _4M |.
          WHEN rb_equip.
            rs_var-report = |{ sy-repid } _4E |.
        ENDCASE.
      WHEN OTHERS.
...
CLASS lcl_qrgen_viewer IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD sort_data.
    FIELD-SYMBOLS <ft_data> TYPE lcl_qrgen_data=>tt_data.
    ASSIGN md_data_table->* TO <ft_data>.
    IF <ft_data> IS ASSIGNED.
      SORT <ft_data> BY werks.
    ENDIF.
  ENDMETHOD.                    "sort_data
  ...
  METHOD fill_fcat.
    ...
    LOOP AT rt_fcat ASSIGNING <fs_fcat>.
      ...
      IF  <fs_fcat>-fieldname = 'FL_CAZ' OR
          <fs_fcat>-fieldname = 'FL_ALL_OBLIG_FIELDS_FILLED' OR
          <fs_fcat>-fieldname = 'EQUNR_EQKTX' OR
          <fs_fcat>-fieldname = 'ADRNR' OR
          <fs_fcat>-fieldname = 'CLINT_UP' OR
          <fs_fcat>-fieldname = 'KLART_UP' OR
          <fs_fcat>-fieldname = 'CLINT' OR
          <fs_fcat>-fieldname = 'KLART' OR
          <fs_fcat>-fieldname = 'MATNR_MAKTX' OR
          <fs_fcat>-fieldname = 'MATNR_MSEHT' OR
          <fs_fcat>-fieldname = 'MATNR_MSEHL' OR
          <fs_fcat>-fieldname = 'ZZETALONMAT_MAKTX' OR
          <fs_fcat>-fieldname = 'ZZETALONMAT_MSEHT' OR
          <fs_fcat>-fieldname = 'ZZETALONMAT_MSEHL' OR
          <fs_fcat>-fieldname = 'EQUIPMENT_CONDITION' OR
          <fs_fcat>-fieldname = 'ROWCOLOR'.
        IF sy-uname <> 'FILIPPOV-ASf'.
          <fs_fcat>-tech = abap_true.
        ENDIF.
      ENDIF.

      CASE <fs_fcat>-fieldname .
        WHEN 'CLASS_UP'. "Вышестоящий класс
        ...
        WHEN 'ZZETALONMAT_MSEHI'. "ЕИ эталона
          <fs_fcat>-scrtext_s = text-f46.
          <fs_fcat>-scrtext_m = text-f47.
          <fs_fcat>-scrtext_l = <fs_fcat>-coltext = text-f48.
        WHEN 'MATNR_MSEHI'. "ЕИ складируемого материала
          <fs_fcat>-scrtext_s = text-f13.
          <fs_fcat>-scrtext_m = text-f14.
          <fs_fcat>-scrtext_l = <fs_fcat>-coltext = text-f15.
        WHEN 'MATNR_ACTUAL_QUANTITY'. "Фактическое количество
          <fs_fcat>-scrtext_s = text-f16.
          <fs_fcat>-scrtext_m = text-f17.
          <fs_fcat>-scrtext_l = <fs_fcat>-coltext = text-f18.
        WHEN 'ETALON_ACTUAL_QUANTITY'. "Фактическое количество в ЕИ эталона
          <fs_fcat>-scrtext_s = text-f19.
          <fs_fcat>-scrtext_m = text-f20.
          <fs_fcat>-scrtext_l = <fs_fcat>-coltext = text-f21.

        WHEN 'EQUIPMENT_CONDITION_TEXT'.
          fcat_copy_text_from_field(
            EXPORTING iv_fieldname = 'EQUIPMENT_CONDITION'
                      it_fcat = rt_fcat
            CHANGING cs_fcat = <fs_fcat> ).
      ENDCASE.
    ENDLOOP.
  ENDMETHOD.                    "fill_fcat
  ...
  METHOD handle_user_command.
    DATA lo_excel TYPE REF TO lcl_qrgen_excel.
    CASE e_ucomm.
    ...
    IF <fs_tab> IS ASSIGNED.
      CASE e_column-fieldname.
        ...
        WHEN 'EQUNR'.
          ...
          ASSIGN COMPONENT e_column-fieldname OF STRUCTURE <fs_tab> TO <fs_field_val>.
          IF <fs_field_val> IS ASSIGNED.
            lcl_f2=>f2_ie03( EXPORTING iv_equnr = <fs_field_val> ).
          ENDIF.
      ENDCASE.
    ENDIF.
